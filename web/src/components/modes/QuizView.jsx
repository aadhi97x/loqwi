...
export default function QuizView({ quiz, onSelect }) {
  if (!quiz) return null;
  ...
  if (quiz.finished) {
    return (
      ...
      >
        ...
        <h2 className="mb-2 text-2xl font-bold text-white">Quiz Khatam!</h2>
        <div className="text-4xl font-extrabold text-brand-300">
          {quiz.score} / {quiz.total}
        </div>
        <p className="mt-1 text-slate-400">sahi jawab</p>
      </motion.div>
    );
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mx-auto w-full max-w-3xl">
      <div className="mb-5 flex items-center gap-3">
        <ListChecks size={26} className="text-brand-300" />
        <h2 className="flex-1 text-2xl font-bold text-white">
          Sawaal {quiz.index + 1} / {total}
        </h2>
        <Badge tone="brand">Score: {quiz.score}</Badge>
      </div>

      <p className="mb-6 text-xl font-semibold text-white">{q.question_text}</p>

      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {q.options.map((opt, i) => {
          const isCorrect = revealed && i === revealed.correctIndex;
          const isWrongPick = revealed && i === revealed.selectedIndex && i !== revealed.correctIndex;
          return (
            <button
              key={i}
              disabled={!!revealed}
              ...
              className={clsx(
                ...
                isCorrect
                  ...
                  : 'border-surface-500 bg-surface-700/70 text-slate-100 hover:border-brand-400'
              )}
            >
      ...
      {revealed ? (
        ...
        <p className="mt-5 text-sm text-slate-500">Bolkar jawab dein: "A", "B", "C" ya "D"</p>
      )}
    </motion.div>
  );
}
