    ...
    <footer className="border-t border-surface-700 bg-surface-900/80 px-4 py-4 backdrop-blur sm:px-6">
      <div className="mx-auto flex max-w-3xl flex-col gap-3">
        <div className="flex flex-wrap items-center justify-center gap-2">
          {MODE_BUTTONS.map(({ key, label, icon: Icon }) => (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full border border-surface-600 bg-surface-800/70 px-3.5 py-1.5 text-sm font-medium text-slate-300 transition-colors hover:border-brand-400 hover:text-white"
            >
              <Icon size={14} />
              {label}
            </button>
        ...
        >
          ...
          />
          <MicButton state={micState} disabled={handsFree || !micSupported} onClick={onToggleMic} size="sm" />
          <button
            type="submit"
