...
const CONCEPT_RE = /\b(samjha[oa]?|samajha[oa]?|explain|kya\s+hai|kya\s+matlab|meaning\s+of|बताओ|समझाओ|समझा)\b/i;
...
function stripTriggerWords(text, re) {
  return text
    .replace(re, ' ')
    .replace(/\b(question|questions|sawaal|sawal|प्रश्न|सवाल|quiz|on|topic|ke\s+upar|par|ka|ki)\b/gi, ' ')
    .replace(/\b\d{1,2}\b/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}
...
/**
 * Local best-effort classification. Returns { mode, topic, numQuestions,
 * targetLang, confident } — confident=false means the caller should consider
 * a backend fallback (classify_intent) instead of acting on this directly.
 */
export function classifyLocal(rawText) {
  ...
  if (TRANSLATE_RE.test(text)) {
    const targetLang = /\b(hindi|हिंदी)\b/i.test(text) ? 'hi' : /\b(english|अंग्रेजी)\b/i.test(text) ? 'en' : null;
    return {
      mode: 'translate',
...
export function parseSpokenOption(text) {
  const lower = ` ${text.toLowerCase()} `;
