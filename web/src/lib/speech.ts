...
//
// SpeechRecognition isn't part of TypeScript's standard DOM lib (it's a
// vendor API, not yet a finished web standard), so we declare just enough of
// its shape here to type-check this file without pulling in a full @types
// package for an API surface we only touch in a handful of places.
...
const RecognitionImpl: SpeechRecognitionConstructor | undefined =
...
export class Listener {
  ...
  private _wantRestart = false;
  private _stoppedByUser = true;
...
export function segmentByScript(text: string): ScriptSegment[] {
  ...
  if (current && currentIsDevanagari !== null)
    segments.push({ text: current, devanagari: currentIsDevanagari });
...
export interface SpeakOptions {
  rate?: number;
  pitch?: number;
  voiceNamePrefs?: { hi?: string; en?: string };
  onSegmentStart?: (seg: ScriptSegment) => void;
}
...
export async function speak(
  text: string,
  { rate = 0.98, pitch = 1, voiceNamePrefs = {}, onSegmentStart }: SpeakOptions = {}
): Promise<void> {
